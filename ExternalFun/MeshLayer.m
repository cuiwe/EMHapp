function [OutputFiles, iSurface] = MeshLayer(TissueFile, TissueLabels, nVertices, erodeFactor, fillFactor, Comment)
...
for iTissue = 1:length(TissueLabels)
    ...
    if(strcmp(TissueLabels{iTissue}, 'CSF'))
        % merge CSF and GM. cuiwei
        mask_gm = (sMri.Cube == sMri.Labels{find(strcmpi(sMri.Labels(:,2), 'Gray')),1});
        mask = (sMri.Cube == sMri.Labels{iLayer,1}) | mask_gm;
    else
    ...
    if ~isempty(Comment)
        ...
        switch (lower(TissueLabels{iTissue}))
            case 'scalp',                 newLabel = 'outer_skin';  SurfaceType = 'Scalp';      TessFile = file_unique(bst_fullfile(SurfaceDir, 'tess_skin_mask.mat'));
            case 'skull',                 newLabel = 'outer_skull'; SurfaceType = 'OuterSkull'; TessFile = file_unique(bst_fullfile(SurfaceDir, 'tess_outerskull_mask.mat'));
            case 'csf',                   newLabel = 'inner_skull'; SurfaceType = 'InnerSkull'; TessFile = file_unique(bst_fullfile(SurfaceDir, 'tess_innerskull_mask.mat'));
            case {'grey','gray','brain'}, newLabel = 'brain';       SurfaceType = 'Cortex';     TessFile = file_unique(bst_fullfile(SurfaceDir, 'tess_cortex_brain_mask.mat'));
            case 'white',                 newLabel = 'white';       SurfaceType = 'Cortex';     TessFile = file_unique(bst_fullfile(SurfaceDir, 'tess_cortex_white_mask.mat'));
            otherwise,                    newLabel = lower(TissueLabels{iTissue});
        end
        sLayer.Comment = sprintf('%s', newLabel);
    end
    ...
    db_surface_type(TessFile, SurfaceType);
    % Return new files
    OutputFiles{end+1} = TessFile;
    bst_progress('inc', 10);
end
% change default, cuiwei
sSubject = bst_get('Subject', iSubject);
if(any(contains({sSubject.Surface.Comment}, 'mid_15002V')))
    db_surface_default(iSubject, 'Cortex', find(contains({sSubject.Surface.Comment}, 'mid_15002V')));
end
if(any(contains({sSubject.Surface.Comment}, 'head mask (10000,0,2')))
    db_surface_default(iSubject, 'Scalp', find(contains({sSubject.Surface.Comment}, 'head mask (10000,0,2')));
end

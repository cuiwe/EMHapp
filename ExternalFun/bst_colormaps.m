...
function SetColorbarVisible(hFig, isVisible) %#ok<DEFNU>
    % Get colorbar and axes handles
    if(strcmp(hFig.Tag, 'MriViewer'))
        hColorbar = findobj(hFig, '-depth', 1, 'Tag', 'Colorbar');
        hAxes     = setdiff(findobj(hFig, '-depth', 1, 'Type', 'axes'), hColorbar);
    else
        hColorbar = findobj(hFig, '-depth', 2, 'Tag', 'Colorbar');
        hAxes     = setdiff(findobj(hFig, '-depth', 2, 'Type', 'axes'),hColorbar);
    end
    ...
    if isVisible && isempty(hColorbar)
        ...
        if(strcmp(hFig.Tag, 'MriViewer'))
            ...
            global hdl % cuiwei
            % To avoid buggy behaviour of OpenGL rendering for colormaps
            hColorbar = axes('Parent', hdl.hpanel, ...
                             'tag',    'Colorbar', ...
                             'XLim',   [0,1], ...
                             'YLim',   [0,256], ...
                             'YTick',  [], ...
                             ...
                             'XTickLabel', []);
            hold(hColorbar, 'on');
            hColorbar.Position = [0.89 0.25 0.03 0.5];
        end
